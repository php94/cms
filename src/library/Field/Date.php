...
use PHP94\Request;
use PHP94\Template;
use PHP94\Form\Field\Date as FieldDate;
use PHP94\Form\Form;
...
class Date implements FieldInterface
{
    ...
    {
        $form->addItem(
            (new FieldDate($field['title'], $field['name'], ''))
        );
    }
    ...
    {
        $form->addItem(
            (new FieldDate($field['title'], $field['name'], $content[$field['name']] ?? ''))->setHelp($field['help'] ?? '')
        );
    }
...
<div>
    <input type="date" class="form-control" name="filter[{$field['name']}][min]" value="{$request->get('filter.'.$field['name'].'.min')}">
</div>
<div>
    <input type="date" class="form-control" name="filter[{$field['name']}][max]" value="{$request->get('filter.'.$field['name'].'.max')}">
</div>
</div>
str;
